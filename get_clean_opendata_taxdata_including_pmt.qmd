...
fname <- "New_York_State,_Local,_and_Local_Purpose_Taxes_and_Fees_Collected_Monthly_by_the_Department_of_Taxation_and_Finance__Beginning_April_1996_20260212.csv"
...
df2 <- df1 |>
  ...
  mutate(
    ...
    value = ifelse(!is.na(fixval_missing), fixval_missing, value)
  ) |>
  relocate(value_original, .after = value)
df2 |> filter(vname == "pit_wh") |> arrange(date) |> filter(year(date) == 2020)
...
df3 <- df2 |>
  ...
  mutate(
    # value_original = value,
    value = ifelse(!is.na(fixval_correction), fixval_correction, value)
  )
...
df4 <- df3 |>
  select(-fixval_missing, -fixval_correction) |>
  mutate(across(c(value, value_original), \(x) x * 1000.))

saveRDS(df4, fs::path(DDATA, "monthly_collections.rds"))
